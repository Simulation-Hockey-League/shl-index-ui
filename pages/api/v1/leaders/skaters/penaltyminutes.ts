...
export default async (
  ...
  const penaltyLeaders = await query(
    SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, t.Name as TeamName, t.Nickname as TeamNickname, t.Abbr as TeamAbbr, p.\`Last Name\` AS Name, s.PIM as PenaltyMinutes, s.GP
    FROM `
    ...
    LEFT JOIN player_rookie_season as prs
      ...
      .append(SQL` ORDER BY PenaltyMinutes `)
      .append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `),
  );

  const parsed = [...penaltyLeaders].map((player) => ({
    id: player.PlayerID,
    name: player.Name,
