...
export default async (
  ...
  const pmLeaders = await query(
    SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, t.Name as TeamName, t.Nickname as TeamNickname, t.Abbr as TeamAbbr, p.\`Last Name\` AS Name, s.plusMinus, s.GP
    FROM `
    ...
    LEFT JOIN player_rookie_season as prs
      ...
      .append(SQL` ORDER BY plusMinus `)
      .append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `),
  );

  const parsed = [...pmLeaders].map((player) => ({
    id: player.PlayerID,
    name: player.Name,
