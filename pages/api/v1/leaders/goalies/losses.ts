...
export default async (
  ...
  const lossLeaders = await query(
    SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, t.Name as TeamName, t.Nickname as TeamNickname, t.Abbr as TeamAbbr, p.\`Last Name\` AS Name, s.Losses, s.GP
    FROM `
    ...
    AND s.SeasonID=${season.SeasonID}`,
      ...
      .append(SQL` ORDER BY s.Losses `)
      .append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `),
  );

  const parsed = [...lossLeaders].map((player) => ({
    id: player.PlayerID,
    name: player.Name,
