...
export default async (
  ...
  const winLeaders = await query(
    SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, t.Name as TeamName, t.Nickname as TeamNickname, t.Abbr as TeamAbbr, p.\`Last Name\` AS Name, s.Wins, s.GP
    FROM `
  ...
  const parsed = [...winLeaders].map((player) => ({
    id: player.PlayerID,
    name: player.Name,
