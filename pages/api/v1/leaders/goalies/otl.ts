...
export default async (
  ...
  const otLeaders = await query(
    SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, t.Name as TeamName, t.Nickname as TeamNickname, t.Abbr as TeamAbbr, p.\`Last Name\` AS Name, s.OT, s.GP
    FROM `
  ...
  const parsed = [...otLeaders].map((player) => ({
    id: player.PlayerID,
    name: player.Name,
